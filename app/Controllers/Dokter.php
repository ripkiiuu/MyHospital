...
class Dokter extends BaseController
{
    ...
    {
        // Cari data Dokter berdasarkan email user yang sedang login
        $email = user()->email;
        $dokterData = $this->dokterModel->where('email', $email)->first();

        // Jika belum sinkron (karena seeder random), kita paksa hubungkan dokter id 1 dengan akun ini sementara
        if (!$dokterData) {
            $this->dokterModel->update(1, ['email' => $email]);
            $dokterData = $this->dokterModel->where('email', $email)->first();
        }
